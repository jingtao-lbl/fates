...
contains
  ...
  subroutine zero_site( site_in )
    ...
    site_in%cstatus(:)       = fates_unset_int    ! are leaves on or off? (per-PFT, #17 Jing Tao)
    site_in%dstatus(:)       = fates_unset_int
    site_in%grow_deg_days(:) = nan  ! growing degree days (per-PFT, #17)
    site_in%snow_depth       = nan
    site_in%nchilldays       = fates_unset_int
    site_in%ncolddays        = fates_unset_int
    site_in%cleafondate(:)   = fates_unset_int  ! doy of leaf on (cold) (per-PFT, #17)
    site_in%cleafoffdate(:)  = fates_unset_int  ! doy of leaf off (cold) (per-PFT, #17)
    site_in%dleafondate(:)   = fates_unset_int  ! doy of leaf on (drought)
    site_in%dleafoffdate(:)  = fates_unset_int  ! doy of leaf off (drought)
    site_in%cndaysleafon(:)  = fates_unset_int  ! days since leaf on (cold) (per-PFT, #17)
    site_in%cndaysleafoff(:) = fates_unset_int  ! days since leaf off (cold) (per-PFT, #17)
  ...
  subroutine set_site_properties( nsites, sites,bc_in )
    ...
    if ( hlm_is_restart == ifalse ) then
       ...
       do s = 1,nsites
          ...
          sites(s)%cleafondate  (1:numpft) = cleafon    - hlm_day_of_year   !Jing Tao (#17): per-PFT cold phenology
          ...
          sites(s)%grow_deg_days(1:numpft) = GDD   !Jing Tao (#17): per-PFT
          ...
          sites(s)%cstatus(1:numpft) = cstat   !Jing Tao (#17): per-PFT (uniform init = bit baseline)
      ...
      pft_loop: do pft =  1, numpft
         if_use_this_pft: if (use_pft_local(pft) .eq. itrue) then
            ...
            if_spmode: if (hlm_use_sp == itrue) then 
               ...
               if (prt_params%season_decid(pft) == itrue .and.                 &   !Jing Tao (#17): per-PFT cold status
